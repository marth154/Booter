...
<div class="stage"></div>
<span class="bar">
	<div class="ui segment" style="float: right" id="scoreUtil">
  ...
  g.Snake.prototype.update = function() {
    ...
    if( this.updateTick >= this.updateTickMax ) {
      ...
      if( this.parentState.grid.get( this.tiles[ 0 ].col, this.tiles[ 0 ].row ) == 'snake' ) {
        var best = <?= $best ?>;
        if(best < document.getElementById("score").innerHTML){
        	
		   $.ajax({
		      url: '../traitement/traitement_jeux.php?snake&best='+document.getElementById("score").innerHTML,
		      type: 'post',
...
<script>
	...
	function ()
	{
	$('#scoreboard').load('../script/scoreJeux.php?jeu=1');
}, 1500);
</script>
